...
test_set_value (void)
{
	...
	printf ("...with same value\n");
	nih_alloc_set_destructor (event->value, destructor_called);
	was_called = 0;
	value = event->value;
	event_set_value (event, "foo");

	/* Value should still be that set */
	if (strcmp (event->value, "foo")) {
		printf ("BAD: event value wasn't what we expected.\n");
		ret = 1;
	}

	/* Value should be actual pointer */
	if (event->value != value) {
		printf ("BAD: event value was changed unexpectedly.\n");
		ret = 1;
	}

	/* Original value should not have been freed */
	if (was_called) {
	...
	printf ("...with different value\n");
	event_set_value (event, "bar");
