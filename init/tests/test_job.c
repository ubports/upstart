...
test_change_goal (void)
{
	...
	/* Check that starting a job with a cause set replaces it with NULL.
	 */
	...
	/* Check that starting a job with a cause passed sets the cause in
	 * the job.
	 */
	...
	/* Check that stopping a job with a cause event set replaces it
	 * with NULL.
	 */
	TEST_FEATURE ("with existing cause");
	event = event_new (NULL, "foo", NULL, NULL);
	...
	/* Check that stopping a job passing a cause sets the job's cause.
	 */
	TEST_FEATURE ("with existing cause");
	event = event_new (NULL, "foo", NULL, NULL);
...
test_change_state (void)
{
	...
	job->start_on = event_operator_new (job, EVENT_MATCH, "wibble", NULL);
	job->stop_on = event_operator_new (job, EVENT_MATCH, "wibble", NULL);
	...
	TEST_ALLOC_FAIL {
		...
		TEST_EQ (job->start_on->value, FALSE);
		TEST_EQ_P (job->start_on->event, NULL);
	 ...
	 * many times in a given period then it is caught, a message is
	 * output and the job sent back to stopped and waiting.  We get
	 * a stopped event, and any blocks and references on start or stop
	 * events are cleared.
	 */
...
test_child_reaper (void)
{
	...
	job->stop_on = event_operator_new (job, EVENT_MATCH, "foo", NULL);
